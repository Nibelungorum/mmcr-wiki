...
注意这台机器**没有** `controller(...)` 调用——控制器方块 ID 与纹理全部走默认。对照 [怪物农场](怪物农场)，那台机器显式配置了控制器朝向。
...
**为什么用**：这台机器的外壳大量使用深板岩砖楼梯和磨制深板岩楼梯做倒角。楼梯有 `facing`、`half`、`shape`、`waterlogged` 四个属性。如果不校验状态，玩家把楼梯朝反了结构照样成型，圆顶会变成一堆乱七八糟的斜面。状态敏感把"外形正确"变成成型的硬条件。
...
`X` 在结构里出现了几十次，整个内壁与堆芯中心都是它。这一个谓词让每个 `X` 位置**六选一**：五种端口族任意一个，或者浅蓝冰，即 `machineBasicBlock` 声明的基础方块外观。
...
- **并行**：`MachineBuilder` 上没有 `parallelizable(true)` / `maxParallelism(...)` / `allowMultithreading()`，配方上也没有 `parallelized(true)`。结构里也没放 `InterfacePredicates.parallelControllers()`。想加并行，看 [高炉](高炉)，那台机器把并行控制器、多线程工厂、并行上限三件套全开了。
...
- **端口等级**：没有 `portTiers(...)`，所以任意等级的端口都能用。高炉 声明了 NORMAL 起步，见 [`PortTiers`](../API/JavaAPI#porttiers)。
...
高炉 关心"跑得快"，反应堆 关心"长得对、IO 全"。两者放一起，`MachineStructureBuilder` 与 `MachineRecipeBuilder` 的常用面基本覆盖完了。
...
- [高炉](高炉) — 并行、多线程与端口等级的对照。
- [怪物农场](怪物农场) — 状态不敏感结构与 `tag(...)` 标签匹配的对照。
- [紫珀炉](紫珀炉) — 另一台大型状态敏感结构，加上智能接口动态调参。
- [蒸馏塔](蒸馏塔) — 多阶段结构与概率输出。
