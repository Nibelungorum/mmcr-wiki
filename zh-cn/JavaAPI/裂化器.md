...
本文拆解 [CRACKER.java](https://github.com/Nibelungorum/ModularMachinery-Community-Refoxed/blob/main/src/main/java/org/nibelungorum/builtin/CRACKER.java)。裂化器是一台 4 层高的多方块机器，它在 MMCR 内置示例中专门演示**自定义控制器（ControllerSpec）**和**流体输出（FluidOutput）**。与高炉的"配方同时跑多份"不同，裂化器的设计目标是：在结构中加入 4 层精细的方块约束、给控制器声明独立的注册 ID 与朝向、并让配方同时产出物品和流体。

## 概览

裂化器把 8 个青金石与 1 个煤炭"裂解"成 500 mB 水与 2 个钻石，这一类"高温高压"的配方语义被映射到多层多方块：外层是磨制闪长岩壳、内层是磨制安山岩芯，再叠蓝冰与青金石块作为隔热层与催化层。它的代码演示三件事：
...
裂化器在机器定义阶段的核心是 `.controller(builder -> ...)`，其余属性保持默认：
...
> 💡 为什么不用 `MachineBuilder.appearance(...)`？合金炉与高炉用 `.appearance(...)` 声明"未成型外观方块"，但裂化器没启用外观定制，所有外观纹理走 MMCR 默认回退。如果你只关心"控制器朝向"，可以省略 `.appearance(...)`，只配置 `.controller(...)`。

## 多方块结构

裂化器的结构是 4 层 × 3 × 3，比前两台示例机器更高：
...
`PatternBuilder` 要求同一 `layer(...)` 调用内各行宽度一致，不同 `layer(...)` 调用之间宽度与行数也必须一致。裂化器的 4 层全部是 3×3，行数（每层 3 行）、宽度（每行 3 字符）都一致，这是机器能正常匹配的前提。如果某层多一行或少一个字符，`build()` 时抛 `IllegalArgumentException`。详见 [`PatternBuilder`](../API/JavaAPI#patternbuilder)。

## 配方

裂化器的内置配方是 8 个青金石 + 1 个煤炭 → 500 mB 水 + 2 个钻石：
...
> 💡 如果想声明**流体输入**（让机器从流体端口取水），使用 `.inputFluid(Fluids.WATER, amount)`，裂化器目前只用流体输出，因为 `anyFluidInput()` 没有被 B 字符接受。

## 三个阶段的协作关系

与 [高炉](高炉) 和 [合金炉](合金炉) 完全相同的三阶段路径：
...
接下来可以阅读 [热能冶炼炉](热能冶炼炉) 看等级系统（LevelType / MachineLevel / LevelModifier）如何用"线圈方块"动态影响配方；或者回到 [高炉](高炉) 对照并行与多线程的另一种实现思路；或者去 [API 参考](../API/JavaAPI) 浏览全部 API。