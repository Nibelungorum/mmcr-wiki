...
本文是 MMCR 网络通信系列的第三个示例。我们逐行拆解 [NETWORK_CENTER_MACHINE.java](https://github.com/Nibelungorum/ModularMachinery-Community-Refoxed/blob/main/src/main/java/org/nibelungorum/builtin/NETWORK_CENTER_MACHINE.java) 的源代码，看看一台作为 "center"（中心/HUB）的多方块机器是如何接收来自 [NETWORK_PRODUCER_MACHINE](../JavaAPI/算力生产者) 的算力上报、维护一份"按节点哈希分桶"的算力表、并把总功率渲染到屏幕与 JADE 上的。
...
- 第一个参数 `REPORT_POWER` 是 `Identifier` 类型的请求 ID，对应 [NETWORK_PRODUCER_MACHINE](../JavaAPI/算力生产者) 中的 `REPORT_POWER` 常量。
...
- vs [NETWORK_PRODUCER_MACHINE](../JavaAPI/算力生产者)：生产者用 `sendRequest` 主动推，中心用 `requestProcess` 被动接。两者必须**双向白名单**。
- vs [DATA_STORAGE_MACHINE](../JavaAPI/数据存储测试机器)：DATA_STORAGE 把 `DataStorage` 当本地计数器；NETWORK_CENTER 把 `DataStorage` 当**网络聚合表**——键名按 peer hash 分桶。
- vs [BLAST_FURNACE](../JavaAPI/高炉)：完全不同的范式——配方 vs 直 tick、单机 vs 网络。
...
- [NETWORK_PRODUCER_MACHINE](../JavaAPI/算力生产者) — 同一网络的对端，主动发送请求。
