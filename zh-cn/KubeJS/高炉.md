...
本文是第一个 KubeJS 示例，演示如何用 KubeJS 注册一台带并行控制器与多端口的高炉。
...
ServerEvents.recipes( event => {
    ...
    event.custom({
        ...
        recipe_pool: 'mmcr_kubejs:kubejs_blast_furnace', // Required machine identifier.
...
`ServerEvents.recipes(...)` 是 KubeJS 自带的原版配方事件。MMCR 通过自定义的 `type: 'mmcr:machine_recipe'` 注册配方类型。

KubeJS 最终把 JSON 数据交给 [`MachineRecipeSchema`] 解析。
...
`event.custom(...).id(...)` 给配方设置 ID。`.id(...)` 是 KubeJS 的链式调用，返回的事件对象仍是普通事件，可以继续链式操作或忽略返回值。
