...
- **`req.peer()` 返回 `MachineReference`**，即 record `(Identifier type, long hash)`。`type` 是机器 ID（这里是 `NETWORK_PRODUCER_MACHINE`），`hash` 是该机器实例的稳定哈希（成型后生成，跨重启保持）。
...
每 tick 接收 200 FE。注意中心要远高于生产者（生产者接收 100），因为它承担聚合职责。
