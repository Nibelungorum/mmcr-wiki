...
太空电梯 与 [高炉](高炉) 的关键差异在于**机器的物理摆放**与**端口的数量/等级约束**：

- 高炉 的控制器只能水平朝向，玩家摆错朝向结构就成型失败。
- 太空电梯 通过 `.allowVerticalFacing()` + `.fullyRotationallySymmetric()` 让控制器可以竖直放置。
...
太空电梯 的组合是 `.allowVerticalFacing() + .fullyRotationallySymmetric()`：
