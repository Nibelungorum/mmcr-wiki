...
接下来是结构注册。高炉 的结构是一个 3×3×3 的多方块，外层是紫珀柱（基础方块），中心是控制器，内壁在不同面上分别支持并行控制器、物品 / 能量端口与熔炉：
...
高炉 的代码看起来很短，但它把 MMCR 的三阶段注册模型完整演示了一遍：
...
接下来可以阅读 [高炉](../KubeJS/高炉) 看同样的高炉在 KubeJS 端怎么实现，并对照 Java API 的 [`MachineBuilderJS`](../API/KubeJS#machinebuilderjs) 等对应方法。或者去 [API 参考](../API/开始) 浏览全部 API。