...
本文是 MMCR 网络通信系列的第二个示例。我们逐行拆解 [NETWORK_PRODUCER_MACHINE.java](https://github.com/Nibelungorum/ModularMachinery-Community-Refoxed/blob/main/src/main/java/org/nibelungorum/builtin/NETWORK_PRODUCER_MACHINE.java) 的源代码，看看一台带网络接口的"算力发电机"是如何把能量数据周期性推送给 [网络中心](../JavaAPI/网络中心) 的。

## 机器简介

算力生产者 是一台**直 tick 机器 + 网络节点**。它站在 "producer"（生产者）一端：
...
6. 如果连续 30 秒无水，就**爆炸**。这是演示"机器内部的退化状态"如何与屏幕提示联动。
...
跟 [数据存储测试机器](../JavaAPI/数据存储测试机器) 一样走直 tick 路径。详见 [`TickBehavior`](../API/JavaAPI#tickbehavior)。
...
每 tick 尝试消耗 100 FE。这里没用二分查找，因为输入固定 100，直接 `simulate()` + `commit()` 两步走。如果失败，`feOk = false`，下游会降级。
...
`sendRequest` 内部把请求入队到当前 tick 的网络处理队列，目标机器的 `requestProcess(REPORT_POWER, ...)` 会在下一个 tick 处理。**生产者不需要阻塞等待响应**，这是典型的"推消息"模型。
...
- vs [数据存储测试机器](../JavaAPI/数据存储测试机器)：DATA_STORAGE 完全没有网络通信，所有读写都在自己机器的 `DataStorage` 里。NETWORK_PRODUCER 把 `DataStorage` 的字段当网络协议字段用，引入了 [`NetworkApi`](#)、[`RequestBody`](#)、`MachineReference` 等新概念。
- vs [网络中心](../JavaAPI/网络中心)：生产者主动 `sendRequest`，中心被动 `requestProcess`。两者必须**双向白名单**才能通信。
- vs [高炉](../JavaAPI/高炉)：高炉 演示了配方驱动的标准三阶段模型，NETWORK_PRODUCER 完全不走配方，只用 `networkInterface` + `tickBehavior`。
- vs KubeJS 端：KubeJS 版 [算力-网络交互示例](../KubeJS/算力-网络交互示例) 演示同一思路在脚本端的写法，`networkInterfaces(ctx)`、`sendRequest(...)`、`dataValue(...)` 是 Java 端 `NetworkApi.interfaces`/`NetworkApi.sendRequest`/`DataValue.of(...)` 的对应物。

## 延伸阅读

- [网络中心](../JavaAPI/网络中心) — 同一网络的对端，处理 `REPORT_POWER` 请求。
- [数据存储测试机器](../JavaAPI/数据存储测试机器) — 理解 `DataStorage` 的最简样本。
