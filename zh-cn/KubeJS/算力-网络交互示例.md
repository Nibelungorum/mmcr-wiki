...
`waterRequirement` 是在脚本顶层 `api.fluidInputRequirement("minecraft:water", 100)` 构造的"100 mB 水输入需求"，这比在 tick 回调里**复用同一个需求对象**比每 tick 重新构造更省事，且语义不变。
...
`ctx.controllerPos()` 返回控制器方块位置。`ctx.level()` 是服务端世界。30 秒没水直接 `level.explode(...)` 半径 4、破坏方块。
