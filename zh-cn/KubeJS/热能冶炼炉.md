...
# A_Level_Machine — 等级系统（热能冶炼炉）

本文演示 MMCR 的 **Level 系统**：机器结构里有一个"等级槽位"，槽位上摆哪一块代表方块决定当前机器等级，不同等级叠加不同的配方修饰器。代码对应 [Java 端的 THERMAL_SMELTING_FURNACE](../JavaAPI/热能冶炼炉)。
...
打开 `startup_scripts/` 目录下的 [A_Level_Machine.js](https://github.com/Nibelungorum/ModularMachinery-Community-Refoxed/blob/main/example/startup_scripts/A_Level_Machine.js)：
...
打开 `server_scripts/structure/` 目录下的 [A_Level_Machine.js](https://github.com/Nibelungorum/ModularMachinery-Community-Refoxed/blob/main/example/server_scripts/structure/A_Level_Machine.js)：
...
打开 `server_scripts/recipe/` 目录下的 [A_Level_Machine.js](https://github.com/Nibelungorum/ModularMachinery-Community-Refoxed/blob/main/example/server_scripts/recipe/A_Level_Machine.js)：
...
A_Level_Machine 与 [THERMAL_SMELTING_FURNACE](../JavaAPI/热能冶炼炉) 同构：Java 端 `LevelType.builder().displayNameKey(...)` + `LevelTypeRegistry.register(...)` ↔ KubeJS 端 `event.createLevelType(...).displayNameKey(...).register()`；Java 端 `LevelSlot.of(typeId)` ↔ `api.levelSlot(typeId)`；Java 端 `MachineRecipeBuilder.requiresLevel(...)` ↔ 数据驱动 `level_requirements` 字段。
...
参见 [JavaAPI/THERMAL_SMELTING_FURNACE](../JavaAPI/热能冶炼炉)。Java 端用 `LevelType.builder()` 流式 API；KubeJS 端用 `event.createLevelType(...).register()` 把所有这些压成几行。
...
- [JavaAPI/THERMAL_SMELTING_FURNACE](../JavaAPI/热能冶炼炉) — 同一台热能冶炼炉的 Java 实现。
