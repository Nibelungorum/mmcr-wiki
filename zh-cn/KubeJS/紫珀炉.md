...
多线程并发跑同一机器时，共享接口值可以避免每个线程独立缓存导致的不一致。
...
- `.runningSound('minecraft:block.furnace.fire_crackle')` —— 注册机器运行期间循环播放的音效 ID。格式是普通 Minecraft 声效 ID（`命名空间:路径`），不限制为 MC 原生声效；玩家可在游戏内的机器 GUI 关闭。详见 [高炉](./高炉) 的同名调用。
- `.finishSound('minecraft:entity.ender_dragon.growl')` —— 注册每条配方完成时单次播放的音效 ID。ID 格式同 `runningSound`；运行期才会发出，不影响结构成型或配方校验。
- `.allowParallelism()` —— 启用并行控制器，允许玩家在同一多方块上安装并行控制器方块以提升单位时间产出。开关默认关闭；未启用时结构层即便摆上并行控制器也不会生效。
- `.maxParallelAmount(32)` —— 把并行上限设到 `32`，与 [热能冶炼炉](./热能冶炼炉) 同值；上限只决定并行控制器能堆到多高，不增加 CPU 调度成本。本教程用 `32` 是因为紫珀炉走的是"配方按外部信号选择"路线，单配方占用率高，不需要堆到 `int` 上限（[高炉](./高炉) 用的 `2147483647` 是给全吞吐量配方留余量）。
...
机器定义不可热加载：`register()` 在 `MMCREvents.startup` 回调内执行，启动期结束后注册窗口即关闭；`/kubejs reload` 只重载 `server_scripts`（结构、配方、事件），不重跑 `startup_scripts`。要改 `runningSound` / `maxParallelAmount` 等参数必须重启游戏。详见 [API/KubeJS#mmcrstartupeventjs](../API/KubeJS#mmcrstartupeventjs)。
...
打开 `server_scripts/recipe/` 目录下的 [A_Smart_Machine.js](https://github.com/Nibelungorum/ModularMachinery-Community-Refoxed/blob/main/example/server_scripts/recipe/A_Smart_Machine.js)：
...
紫珀炉 在 [高炉](高炉) 的基础上加了"配方按外部信号选择"的能力。启动期新增 `.smartInterface(...)...end()` × 2、`.XxxByInterface(...)` × N；结构层新增 `api.smartInterface()` 谓词；配方层新增 `mmcr:smart_interface` 类型条目。

与 [合金炉](合金炉) / [热能冶炼炉](热能冶炼炉) 的对比参见上节表格。三套系统的区别在"调节方式"和"调节粒度"上——Modifier 最自由（任何方块都行）、Level 最受限（必须等级 state 方块）、Smart Interface 最程序化（接口值可由代码或事件触发）。

与 Java 端 [紫珀炉](../JavaAPI/紫珀炉) 对比：两边的链式 API 几乎一一对应；KubeJS 端多一个数据驱动 `mmcr:smart_interface` 配方条目写法。
...
- [高炉](高炉) — 不使用智能接口的基础机器。
- [太空电梯](太空电梯) — HOST + MODULE 教程。
- [热能冶炼炉](热能冶炼炉) — Level 系统教程。
- [合金炉](合金炉) — Modifier 系统教程。