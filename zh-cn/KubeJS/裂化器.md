...
- A_Vertical_Machine 通过 `.allowVerticalFacing()` + `.fullyRotationallySymmetric()` 让控制器可以竖直放置（向上或向下）。
...
:::info 备注
与 [A_Group_Machine](A_Group_Machine) 不同的是，这台机器**没有调用 `.expandableStructure(true)`**——裂化器只有一个固定形态，不需要分组。结构脚本用扁平式 API 即可。
:::
