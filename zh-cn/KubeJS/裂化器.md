...
- 太空电梯 通过 `.allowVerticalFacing()` + `.fullyRotationallySymmetric()` 让控制器可以竖直放置（向上或向下）。

新工具——`portRequirements(...)` 与 `portTierRequirements(...)`：**约束端口的数量和等级下限**。
...
- `.allowVerticalFacing()`：声明控制器**可以竖直放置**（即面朝上 / 面朝下）。
...
默认 `false`。
当玩家把控制器面朝下放置时，正面纹理仍然显示在"面对玩家"的方向——玩家从下往上看也会看到正确纹理。
- `.recipeFamily("mmcr_kubejs:kubejs_cracker")`：配方族 ID。

`builder.register()` 终结，把机器提交到启动期注册窗口。

:::info 备注
与 [蒸馏塔](蒸馏塔) 不同的是，这台机器**没有调用 `.expandableStructure(true)`**——裂化器只有一个固定形态，不需要分组。结构脚本用扁平式 API 即可。
:::
